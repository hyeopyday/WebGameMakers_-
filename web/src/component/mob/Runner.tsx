...
const GOAL_LOCK_TIME = 0.2; // 단축

// 즉시 재도주 트리거
const PANIC_RECALC_DIST = SAFE_INNER;
const PLAYER_PIXEL_TRIGGER = 24; // 픽셀 기반 재계산 트리거

// 스프라이트(아이콘 16x16 단일 프레임)
const SPRITE_W = 16;
const SPRITE_H = 16;
...
function pickEscapePath(
  ...
  const steps = [10, 14, 18, 24, 28]; // 셀 단위
  ...
  useEffect(() => {
    ...
    function panicRecalc() {
      ...
      if ((!newPath.length || pdist < ESCAPE_KEEP) && pdist > 1e-6) {
        ...
        const farCells = 32;
        const gx = clamp(
          Math.floor((state.px + ux * farCells * CELL_SIZE) / CELL_SIZE),
          0,
          grid[0].length - 1
        );
        const gy = clamp(
          Math.floor((state.py + uy * farCells * CELL_SIZE) / CELL_SIZE),
          0,
          grid.length - 1
        );
    ...
    const loop = (t: number) => {
      ...
      const playerMovedCell =
        pcx !== state.lastPlayerCell.x || pcy !== state.lastPlayerCell.y;
      ...
      const closingFast = pdist < state.lastPdist - 12; // 급접근 감지

      // 패닉 우선권: 근접 시 즉시 재계산(타이머/락 무시)
      if (state.havePlayer && state.mode === "escape" && pdist <= PANIC_RECALC_DIST) {
        state.goalLock = 0; // 락 해제
        panicRecalc();
      } else {
        // 일반 재계산 조건
        if (
          ...
          (state.pathTimer >= PATH_RECALC_TIME ||
            playerMovedCell ||
            playerMovedPx > PLAYER_PIXEL_TRIGGER ||
            closingFast ||
            (state.path.length === 0 && pdist < ESCAPE_KEEP) ||
            (state.goalLock <= 0 && pdist <= SAFE_INNER * 1.1))
        ) {
          // goalLock이 남아 있어도 SAFE_INNER 근접 시 무시
          if (pdist <= SAFE_INNER * 1.1) state.goalLock = 0;
          if (state.goalLock <= 0 || pdist <= SAFE_INNER * 1.1) {
            panicRecalc();
          } else if (state.pathTimer >= PATH_RECALC_TIME) {
            panicRecalc();
          }
        }
      }
      ...
      if (state.path.length > 0) {
        ...
        if (d2 < 8) {
          ...
          const moved = moveWithWorldCollision(
            state.px,
            state.py,
            vx,
            vy,
