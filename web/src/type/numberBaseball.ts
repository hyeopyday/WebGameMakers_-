...
// 숫자야구 유틸: 중복 불가, 선행 0 허용

/** guess/secret 모두 length 자리 문자열, 각 자리는 0~9, 중복 없다고 가정 */
export function judgeGuess(guess: string, secret: string): { strike: number; ball: number; out: number } {
  let strike = 0;
  let ball = 0;
...
export function formatAttemptLine(index1: number, guess: string, res: { strike:number; ball:number; out:number }): string {
